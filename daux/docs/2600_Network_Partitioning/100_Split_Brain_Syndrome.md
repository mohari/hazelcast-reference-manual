...
When held in cache, the updates will be considered dirty and will be written to the database when it becomes available. You can view the dirty entry counts per cluster member in the Management Center web console (please see the [Map Monitoring section](http://docs.hazelcast.org/docs/management-center/3.8.3/manual/html/Managing_Maps.html)).
